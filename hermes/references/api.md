...
| `reminder_rules_set` | `PUT /api/v1/subscriptions/{id}/reminder-rules` | `subscriptions:write` | Yes |
| `reminders_claim` | `POST /api/v1/reminders/claim` | `reminders:consume` | No |
| `reminder_ack` | `POST /api/v1/reminders/deliveries/{id}/ack` | `reminders:consume` | No |
| `reminder_fail` | `POST /api/v1/reminders/deliveries/{id}/fail` | `reminders:consume` | No |
| `audit_recent` | `GET /api/v1/audit-logs` | `audit:read` | No |
...
The dedicated Hermes token should normally contain `subscriptions:read`, `subscriptions:write`, `payments:write`, `analytics:read`, and `reminders:consume`. Add `audit:read` only when audit lookup is required. Never grant `tokens:manage`, `reminders:scan`, `reminders:read`, or `reminders:retry`.
