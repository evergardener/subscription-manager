...
export function SubscriptionsPage() {
  ...
  const subscriptions = useQuery({ queryKey: ["subscriptions", query], queryFn: ({ signal }) => listSubscriptions(query, signal) });
  ...
  return (
    <section>
      ...
      <div className="toolbar"><label className="search-field"><span aria-hidden="true">⌕</span><input aria-label="搜索订阅" placeholder="搜索名称或供应商" value={query} onChange={(event) => setQuery(event.target.value)} /></label><div className="segmented" aria-label="布局"><button className={layout === "cards" ? "active" : ""} onClick={() => setLayout("cards")} aria-label="卡片视图">▦</button><button className={layout === "table" ? "active" : ""} onClick={() => setLayout("table")} aria-label="表格视图">☷</button></div></div>
      {subscriptions.isPending ? <LoadingState /> : subscriptions.error ? <ErrorState error={subscriptions.error} /> : subscriptions.data.items.length === 0 ? <EmptyState title={query ? "没有匹配结果" : "还没有订阅"} message={query ? "试试其他关键词。" : "创建第一项订阅，开始跟踪续费与支出。"} /> : layout === "cards" ? <div className="subscription-grid">{subscriptions.data.items.map((item) => <Link className="subscription-card" key={item.id} to={`/subscriptions/${item.id}`}><div className="card-top"><span className="service-mark large">{item.name.slice(0, 1).toUpperCase()}</span><span className={`status-pill ${item.status}`}>{item.status}</span></div><div><h2>{item.name}</h2><p>{item.vendor || "未填写供应商"}</p></div>{item.billing_plan && <div className="card-money"><strong><Money amount={item.billing_plan.amount} currency={item.billing_plan.currency} /></strong><span>下次续费 {item.billing_plan.next_billing_date ?? "—"}</span></div>}</Link>)}</div> : <div className="table-wrap"><table><thead><tr><th>订阅</th><th>状态</th><th>价格</th><th>下次续费</th></tr></thead><tbody>{subscriptions.data.items.map((item) => <tr key={item.id}><td><Link to={`/subscriptions/${item.id}`}>{item.name}</Link><small>{item.vendor}</small></td><td><span className={`status-pill ${item.status}`}>{item.status}</span></td><td>{item.billing_plan ? <Money amount={item.billing_plan.amount} currency={item.billing_plan.currency} /> : "—"}</td><td>{item.billing_plan?.next_billing_date ?? "—"}</td></tr>)}</tbody></table></div>}
