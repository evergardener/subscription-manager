...
import { currencyLabel, getCurrencies, getDefaultCurrency } from "../app/currencyPreferences";
...
export function SubscriptionsPage() {
  ...
  return (
    <section>
      <header className="page-header"><div><p className="eyebrow">Subscriptions</p><h1>订阅</h1><p className="muted">管理价格、续费日期和服务状态。</p></div><button className="primary-button" disabled={offline} onClick={() => { setOpen(true); setParams({ create: "1" }); }}>＋ 新建订阅</button></header>
      ...
      {subscriptions.isPending ? <LoadingState /> : subscriptions.error ? <ErrorState error={subscriptions.error} /> : subscriptions.data.items.length === 0 ? <EmptyState title={query ? "没有匹配结果" : "还没有订阅"} message={query ? "试试其他关键词。" : "创建第一项订阅，开始跟踪续费与支出。"} /> : layout === "cards" ? <div className="subscription-grid">{subscriptions.data.items.map((item) => <Link className={`subscription-card ${item.archived_at ? "archived" : ""}`} key={item.id} to={`/subscriptions/${item.id}`}><div className="card-top"><span className="service-mark large">{item.name.slice(0, 1).toUpperCase()}</span><span className={`status-pill ${item.archived_at ? "archived" : item.status}`}>{item.archived_at ? "已归档" : item.status}</span></div><div><h2>{item.name}</h2><p>{item.vendor || "未填写供应商"}</p></div>{item.billing_plan && <div className="card-money"><strong><Money amount={item.billing_plan.amount} currency={item.billing_plan.currency} /></strong><span>{item.billing_plan.auto_renew ? `下次续费 ${item.billing_plan.next_billing_date ?? "—"}` : "已关闭自动续费"}</span></div>}{item.spend && <span className="muted">累计实付 {Object.entries(item.spend.by_currency).map(([currency, total], index) => <span key={currency}>{index > 0 ? " ＋ " : ""}<Money amount={total} currency={currency} /></span>)}</span>}</Link>)}</div> : <div className="table-wrap"><table><thead><tr><th>订阅</th><th>状态</th><th>价格</th><th>续费安排</th><th>累计实付</th></tr></thead><tbody>{subscriptions.data.items.map((item) => <tr key={item.id}><td><Link to={`/subscriptions/${item.id}`}>{item.name}</Link><small>{item.vendor}</small></td><td><span className={`status-pill ${item.archived_at ? "archived" : item.status}`}>{item.archived_at ? "已归档" : item.status}</span></td><td>{item.billing_plan ? <Money amount={item.billing_plan.amount} currency={item.billing_plan.currency} /> : "—"}</td><td>{item.billing_plan ? item.billing_plan.auto_renew ? item.billing_plan.next_billing_date ?? "—" : "不自动续费" : "—"}</td><td>{item.spend ? Object.entries(item.spend.by_currency).map(([currency, total], index) => <span key={currency}>{index > 0 ? " ＋ " : ""}<Money amount={total} currency={currency} /></span>) : "—"}</td></tr>)}</tbody></table></div>}
...
function SubscriptionDialog({ pending, error, onClose, onSubmit }: { pending: boolean; error: unknown; onClose: () => void; onSubmit: (payload: SubscriptionCreate) => void }) {
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const startDate = value(data, "start_date");
    onSubmit({ name: value(data, "name"), vendor: value(data, "vendor"), status: "active", ...(startDate ? { start_date: startDate } : {}), billing_plan: { amount: value(data, "amount"), currency: value(data, "currency"), interval_unit: value(data, "interval") as "day" | "week" | "month" | "year", interval_count: 1, anchor_date: nextBillingDate, next_billing_date: nextBillingDate, auto_renew: data.get("auto_renew") === "on", billing_mode: "fixed" } });
  }
  const currencies = getCurrencies();
  return <div className="modal-backdrop" role="presentation"><section className="modal" role="dialog" aria-modal="true" aria-labelledby="new-subscription"><div className="panel-heading"><div><p className="eyebrow">New subscription</p><h2 id="new-subscription">新建订阅</h2></div><button className="icon-button" onClick={onClose} aria-label="关闭">×</button></div><form onSubmit={submit}><div className="form-grid"><label>名称<input name="name" required maxLength={200} autoFocus /></label><label>供应商<input name="vendor" maxLength={200} /></label><label>金额<input name="amount" required type="number" min="0" step="0.01" /></label><label>币种<select name="currency" defaultValue={getDefaultCurrency(currencies)}>{currencies.map((currency) => <option key={currency} value={currency}>{currencyLabel(currency)}</option>)}</select></label><label>周期<select name="interval" defaultValue="month"><option value="month">每月</option><option value="year">每年</option><option value="week">每周</option><option value="day">每天</option></select></label><label>下次续费<input name="next_billing_date" type="date" min={today} defaultValue={today} required /></label><label>开始日期（可选）<input name="start_date" type="date" /></label><label className="cache-toggle"><input name="auto_renew" type="checkbox" defaultChecked />到期后继续自动续费</label></div>{error ? <ErrorState error={error} /> : null}<div className="modal-actions"><button className="secondary-button" type="button" onClick={onClose}>取消</button><button className="primary-button" disabled={pending} type="submit">{pending ? "正在创建…" : "创建订阅"}</button></div></form></section></div>;
}
