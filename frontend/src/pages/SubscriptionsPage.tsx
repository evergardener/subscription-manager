...
export function SubscriptionsPage() {
  ...
  return (
    <section>
      ...
      {subscriptions.isPending ? <LoadingState /> : subscriptions.error ? <ErrorState error={subscriptions.error} /> : subscriptions.data.items.length === 0 ? <EmptyState title={query ? "没有匹配结果" : "还没有订阅"} message={query ? "试试其他关键词。" : "创建第一项订阅，开始跟踪续费与支出。"} /> : layout === "cards" ? <div className="subscription-grid">{subscriptions.data.items.map((item) => <Link className={`subscription-card ${item.archived_at ? "archived" : ""}`} key={item.id} to={`/subscriptions/${item.id}`}><div className="card-top"><span className="service-mark large">{item.name.slice(0, 1).toUpperCase()}</span><span className={`status-pill ${item.archived_at ? "archived" : item.status}`}>{item.archived_at ? "已归档" : item.status}</span></div><div><h2>{item.name}</h2><p>{item.vendor || "未填写供应商"}</p></div>{item.billing_plan && <div className="card-money"><strong><Money amount={item.billing_plan.amount} currency={item.billing_plan.currency} /></strong><span>{item.billing_plan.auto_renew ? `下次续费 ${item.billing_plan.next_billing_date ?? "—"}` : "已关闭自动续费"}</span></div>}</Link>)}</div> : <div className="table-wrap"><table><thead><tr><th>订阅</th><th>状态</th><th>价格</th><th>续费安排</th></tr></thead><tbody>{subscriptions.data.items.map((item) => <tr key={item.id}><td><Link to={`/subscriptions/${item.id}`}>{item.name}</Link><small>{item.vendor}</small></td><td><span className={`status-pill ${item.archived_at ? "archived" : item.status}`}>{item.archived_at ? "已归档" : item.status}</span></td><td>{item.billing_plan ? <Money amount={item.billing_plan.amount} currency={item.billing_plan.currency} /> : "—"}</td><td>{item.billing_plan ? item.billing_plan.auto_renew ? item.billing_plan.next_billing_date ?? "—" : "不自动续费" : "—"}</td></tr>)}</tbody></table></div>}
