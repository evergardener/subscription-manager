...
export function SettingsPage() {
  ...
  function savePreferences(event: FormEvent) { event.preventDefault(); localStorage.setItem("hermes-timezone", timezone); localStorage.setItem("hermes-default-reminder", defaultReminder); }
  ...
  return <section><header className="page-header"><div><p className="eyebrow">Settings</p><h1>设置</h1><p className="muted">会话、显示偏好与自动化访问。</p></div></header>
    <div className="settings-grid"><div className="settings-main">
      <section className="panel"><div className="panel-heading"><div><p className="eyebrow">Preferences</p><h2>显示与区域</h2></div></div><form onSubmit={savePreferences}><div className="form-grid"><label>主题<select value={theme} onChange={(event) => setTheme(event.target.value)}><option value="system">跟随系统</option><option value="light">浅色</option><option value="dark">深色</option></select></label><label>时区<input value={timezone} onChange={(event) => setTimezone(event.target.value)} /></label><label>默认提醒（提前天数）<input value={defaultReminder} pattern="[0-9]+(,[0-9]+)?" onChange={(event) => setDefaultReminder(event.target.value)} /></label></div><div className="modal-actions"><button className="primary-button" type="submit">保存偏好</button></div></form></section>
      <section className="panel"><div className="panel-heading"><div><p className="eyebrow">API access</p><h2>API Token</h2></div><button className="text-button" onClick={() => setDialog(true)}>＋ 创建</button></div><p className="muted">为 Hermes 或自动化客户端创建最小权限、可撤销的独立 Token。</p>{tokens.error ? <ErrorState error={tokens.error} /> : tokens.data?.length ? <div className="token-list">{tokens.data.map((token) => <article key={token.id} className={token.revoked_at ? "revoked" : ""}><div><strong>{token.name}</strong><span>{token.actor_id} · {token.scopes.join(" · ")}</span></div><span>{token.revoked_at ? "已撤销" : token.expires_at ? `到期 ${token.expires_at.slice(0, 10)}` : "长期有效"}</span>{!token.revoked_at && <button className="danger-button" disabled={revoke.isPending} onClick={() => { if (window.confirm(`确认撤销 Token“${token.name}”？此操作不可恢复。`)) revoke.mutate(token.id); }}>撤销</button>}</article>)}</div> : <EmptyState title="没有 API Token" message="Web UI 不需要 Token；仅为 Hermes 或自动化按需创建。" />}</section>
