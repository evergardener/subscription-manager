...
import { commonCurrencies, currencyLabel, getCurrencies, getDefaultCurrency, isSupportedCurrency, saveCurrencyPreferences } from "../app/currencyPreferences";
...
export function SettingsPage() {
  ...
  async function savePreferences(event: FormEvent) { event.preventDefault(); localStorage.setItem("hermes-timezone", timezone); localStorage.setItem("hermes-default-reminder", defaultReminder); localStorage.setItem("hermes-persistent-cache", String(persistentCache)); saveCurrencyPreferences(currencies, defaultCurrency); if (!persistentCache) await clearBusinessCache(); }
  function addCurrency() { const currency = newCurrency.trim().toUpperCase(); if (!isSupportedCurrency(currency)) { setCurrencyError("请输入受支持的 ISO 4217 三字符币种代码。"); return; } if (!currencies.includes(currency)) setCurrencies((current) => [...current, currency]); setNewCurrency(""); setCurrencyError(null); }
  ...
  return <section><header className="page-header"><div><p className="eyebrow">Settings</p><h1>设置</h1><p className="muted">会话、显示偏好与自动化访问。</p></div></header>
    <div className="settings-grid"><div className="settings-main">
      <section className="panel"><div className="panel-heading"><div><p className="eyebrow">Preferences</p><h2>显示与区域</h2></div></div><form onSubmit={(event) => void savePreferences(event)}><div className="form-grid"><label>主题<select value={theme} onChange={(event) => setTheme(event.target.value)}><option value="system">跟随系统</option><option value="light">浅色</option><option value="dark">深色</option></select></label><label>时区<input value={timezone} onChange={(event) => setTimezone(event.target.value)} /></label><label>默认提醒（提前天数）<input value={defaultReminder} pattern="[0-9]+(,[0-9]+)?" onChange={(event) => setDefaultReminder(event.target.value)} /></label><label>新订阅默认币种<select value={defaultCurrency} onChange={(event) => setDefaultCurrency(event.target.value)}>{currencies.map((currency) => <option key={currency}>{currencyLabel(currency)}</option>)}</select></label><fieldset className="currency-options"><legend>新订阅可选币种</legend><div className="scope-grid">{commonCurrencies.map((currency) => <label key={currency}><input type="checkbox" checked={currencies.includes(currency)} disabled={currency === defaultCurrency} onChange={(event) => setCurrencies((current) => event.target.checked ? [...current, currency] : current.filter((item) => item !== currency))} />{currencyLabel(currency)}</label>)}</div><div className="currency-add"><label>添加币种代码<input value={newCurrency} maxLength={3} placeholder="例如 SGD" onChange={(event) => { setNewCurrency(event.target.value.toUpperCase()); setCurrencyError(null); }} /></label><button className="secondary-button" type="button" onClick={addCurrency}>添加</button></div>{currencyError && <p className="form-error" role="alert">{currencyError}</p>}<div className="currency-selection">{currencies.filter((currency) => !commonCurrencies.includes(currency as typeof commonCurrencies[number])).map((currency) => <span key={currency}>{currencyLabel(currency)}<button type="button" aria-label={`移除 ${currency}`} disabled={currency === defaultCurrency} onClick={() => setCurrencies((current) => current.filter((item) => item !== currency))}>×</button></span>)}</div></fieldset><label className="cache-toggle"><input type="checkbox" checked={persistentCache} onChange={(event) => setPersistentCache(event.target.checked)} />允许在此设备保存最近只读数据</label></div><div className="modal-actions"><button className="primary-button" type="submit">保存偏好</button></div></form></section>
