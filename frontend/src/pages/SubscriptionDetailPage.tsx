...
export function SubscriptionDetailPage() {
  ...
  const dates = useMutation({ mutationFn: (values: ServiceDates & { start_date: string | null }) => { const { start_date, ...serviceDates } = values; return updateServiceDates(subscription.data as Subscription, serviceDates, start_date); }, onSuccess: refresh });
  ...
  return <section>
    ...
    <div className="detail-grid"><div className="detail-main">
      ...
      <section className="panel"><div className="panel-heading"><div><p className="eyebrow">Payments</p><h2>付款记录</h2></div><button className="text-button" onClick={() => setDialog("payment")}>添加</button></div>{item.spend ? <p className="spend-total"><strong>累计实付：</strong>{Object.entries(item.spend.by_currency).map(([currency, total], index) => <span key={currency}>{index > 0 ? " ＋ " : ""}<Money amount={total} currency={currency} /></span>)}<span className="muted">（共 {item.spend.payment_count} 笔）</span></p> : null}{payments.data?.length ? <div className="payment-list">{payments.data.map((entry) => <article key={entry.id}><div><strong><Money amount={entry.amount} currency={entry.currency} /></strong><span>{new Intl.DateTimeFormat("zh-CN", { dateStyle: "medium", timeStyle: "short" }).format(new Date(entry.paid_at))}</span></div><span>{entry.source}</span></article>)}</div> : <EmptyState title="尚无付款" message="记录实际付款后，统计会与预计金额分开展示。" />}</section>
      ...
      <section className="panel"><div className="panel-heading"><div><p className="eyebrow">Dates</p><h2>订阅周期与关键日期</h2></div><button className="text-button" disabled={offline} onClick={() => setDialog("dates")}>编辑</button></div><div className="date-list"><span><small>开始日期</small>{item.start_date ?? "—"}</span><span><small>停止续费到期</small>{item.service_dates?.service_expiry_date ?? "—"}</span><span><small>试用结束</small>{item.service_dates?.trial_end_date ?? "—"}</span><span><small>取消截止</small>{item.service_dates?.cancellation_deadline ?? "—"}</span><span><small>合同结束</small>{item.service_dates?.contract_end_date ?? "—"}</span></div></section>
    ...
    {dialog === "dates" && <DatesDialog initial={item.service_dates} startDate={item.start_date ?? null} pending={dates.isPending} error={dates.error} onClose={() => setDialog(null)} onSubmit={(values) => dates.mutate(values)} />}
...
function PaymentDialog({ currency, amount, event, autoRenew, pending, error, onClose, onSubmit }: { currency: string; amount: string; event?: EventItem; autoRenew: boolean; pending: boolean; error: unknown; onClose: () => void; onSubmit: (value: { amount: string; currency: string; paid_at: string; notes?: string; billing_event_id?: string; advance_schedule: boolean }) => void }) {
  ...
  return <DialogFrame title="记录实际付款" pending={pending} error={error} onClose={onClose} submitLabel="记录付款" onSubmit={(data) => { const advance_schedule = data.get("advance_schedule") === "on"; onSubmit({ amount: field(data, "amount"), currency, paid_at: new Date(field(data, "paid_at")).toISOString(), notes: field(data, "notes"), billing_event_id: advance_schedule ? event?.id : undefined, advance_schedule }); }}><div className="form-grid"><label>金额<input name="amount" type="number" min="0.01" step="0.01" required defaultValue={Number(amount).toFixed(2)} /></label><label>币种<input value={currency} disabled /></label><label>付款时间<input name="paid_at" type="datetime-local" required defaultValue={now.toISOString().slice(0, 16)} /></label><label>备注<input name="notes" maxLength={500} /></label><label className="cache-toggle"><input name="advance_schedule" type="checkbox" defaultChecked={autoRenew && Boolean(event)} disabled={!event} />推进到下一账期</label></div>{event ? <p className="muted">关联当前账单事件：{event.event_date}</p> : <p className="muted">没有匹配的当前账单事件，本次付款将作为历史补录且不推进账期。</p>}</DialogFrame>;
}
...
function DatesDialog({ initial, startDate, pending, error, onClose, onSubmit }: { initial?: ServiceDates; startDate: string | null; pending: boolean; error: unknown; onClose: () => void; onSubmit: (value: ServiceDates & { start_date: string | null }) => void }) {
  const nullableDate = (data: FormData, name: string) => field(data, name) || null;
  return <DialogFrame title="编辑订阅周期与关键日期" pending={pending} error={error} onClose={onClose} submitLabel="保存日期" onSubmit={(data) => onSubmit({ start_date: nullableDate(data, "start_date"), trial_end_date: nullableDate(data, "trial_end_date"), service_expiry_date: nullableDate(data, "service_expiry_date"), cancellation_deadline: nullableDate(data, "cancellation_deadline"), contract_end_date: nullableDate(data, "contract_end_date") })}><p className="muted">停止续费到期时间用于计算订阅周期终点；留空可清除日期，保存后对应的未来事件会同步更新。</p><div className="form-grid"><label>开始日期<input name="start_date" type="date" defaultValue={startDate ?? ""} /></label><label>停止续费到期<input name="service_expiry_date" type="date" defaultValue={initial?.service_expiry_date ?? ""} /></label><label>试用结束<input name="trial_end_date" type="date" defaultValue={initial?.trial_end_date ?? ""} /></label><label>取消截止<input name="cancellation_deadline" type="date" defaultValue={initial?.cancellation_deadline ?? ""} /></label><label>合同结束<input name="contract_end_date" type="date" defaultValue={initial?.contract_end_date ?? ""} /></label></div></DialogFrame>;
}
