...
本文件用于指导 Codex 从零实现一套独立的个人订阅与数字服务生命周期管理系统。系统以 Web/PWA 为人工操作界面，以 REST API 为稳定边界，以 Hermes Tool/Skill 为自然语言入口，并通过 n8n 或独立调度任务完成续费、到期和取消截止提醒。
...
- API-first：所有核心操作均通过受控 API 完成，Web UI、Hermes、n8n 共用同一业务层。
...
- 通知适配器 MVP 必须实现 ntfy；独立 scheduler 服务负责扫描和重试，n8n 仅作为可选外部触发器。
...
| 提醒与投递记录 | 必须 | 增强渠道 | MVP 至少一个渠道，必须支持幂等、失败记录和补发。 |
...
| 汇率与统一基准币种统计 | 否 | 是 | MVP 可保存基准币种设置，但不得生成伪换算总额。 |
...
Android / Desktop Browser
  ...
  │       │           │
  ▼       ▼           ▼
PostgreSQL Scheduler  Hermes Tool/Skill
          │           │
          ▼           ▼
      Notification   Natural-language operations
      Adapter / n8n
```
...
| Scheduler | 独立进程扫描到期事件、生成/领取投递、重试和补偿；n8n 可调用扫描 API | 不得自行绕过 service 层修改订阅核心数据 |
| Hermes Tool/Skill | 自然语言解析、调用 API、组织回复 | 不得绕过 API 或自行推断写入关键日期 |
| Notification Adapter | 统一发送 ntfy/Telegram/Email/Webhook | 不得承担业务判断 |
...
| Scheduler | APScheduler 独立进程/容器 | MVP 的唯一调度责任方；n8n 仅可选触发受保护 API。 |
...
3. 若唯一键冲突且状态为 sent，则跳过。

4. 若状态为 failed 且未超过重试上限，则重试并累计 attempt_count。

5. 若系统停机导致错过发送，恢复后在 grace_window_days 内补发，并在文案中标识“补发”。

6. 扫描与投递必须使用数据库原子领取或等价锁机制；不得仅依赖“先查询再发送”，以避免多个调度器并发发送同一通知。
...
| POST | /reminders/scan | 由 scheduler 调用；n8n 可使用 scoped Token 外部触发。 |
...
POST /api/v1/subscriptions
{
  ...
  "reminder_rules": [
    {"event_type": "billing", "offset_days": 5, "channel": "ntfy"},
    {"event_type": "billing", "offset_days": 1, "channel": "ntfy"}
  ]
}
```
...
| Settings | MVP 基础版 | 时区、ntfy 配置、默认提醒、Session 信息和 API Token 管理。 |
...
- MVP 实现 ntfy Adapter；Adapter 接口保留 Telegram、Email 和 Webhook 的后续扩展能力。n8n 不作为通知渠道实现的替代品。

- 通知正文至少包含：服务名、事件类型、日期、金额、自动续费状态、详情链接。

- API 触发扫描必须受 token 保护。

- 通知失败写入 reminder_deliveries.error，不能静默丢失。

- 提供 dry-run 参数，只生成待发送列表而不真正发送。

- 通知适配器必须设置连接与读取超时，记录供应商响应摘要，但不得记录通知凭据或完整敏感响应。

- Channel 凭据只从 secret/environment 或受保护配置读取；不得从普通订阅字段传入。
...
- MVP scope 至少包括 `subscriptions:read`、`subscriptions:write`、`payments:write`、`reminders:scan`、`audit:read`、`tokens:manage`。Hermes 不得拥有 `tokens:manage`，scheduler 默认只有 `reminders:scan`。
...
| Reverse Proxy | 支持 Traefik，给出 label 示例但不硬编码域名。 |
...
| 决策项 | 默认值 |
| --- | --- |
| 基准币种 | CNY，可在设置中修改；MVP 仅用于偏好设置，不触发跨币种换算。 |
| 时区 | 由部署环境配置，默认 Asia/Shanghai；数据库时间使用 UTC。 |
| 提醒扫描 | 每天 08:00；允许 n8n 外部触发。 |
| 默认提醒 | 账单前 7 天和 1 天。 |
| 删除策略 | 仅归档，不提供普通硬删除。 |
| 鉴权 | Web 使用同源 HttpOnly Session；Hermes/scheduler 使用独立 scoped API Token；OIDC 可选。 |
| 通知与调度 | 独立 scheduler 容器 + ntfy；n8n 仅作为可选外部触发。 |
| 部署 | Docker Compose 优先，兼容外部 PostgreSQL。 |
...
| DEC-004 | 调度器与首个通知渠道 | 独立 backend scheduler 容器负责扫描和重试，n8n 仅作为可选外部触发；首个渠道实现 ntfy | 保证核心提醒链路不依赖外部自动化平台；未采用 n8n 作为唯一调度器 | 部署、运维、通知 | 已确认，2026-07-16 |
...
- `DEC-004` 已关闭，明确唯一调度责任方、锁策略、重试参数和凭据来源。
